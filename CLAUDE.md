...
**注意**：此部分仅作为快速参考，详细的 Phase 定义和任务列表请查看 [IMPLEMENTATION_PLAN.md](docs/IMPLEMENTATION_PLAN.md)
...
- [x] 数据库迁移（InterviewSession, InterviewAnswer, MemoryProfile）
- [x] 核心业务逻辑（实体抽取、MemoryProfile 更新、调度器）
- [x] 前端骨架题池（80+ 题目，支持选择题）
- [x] 访谈启动、回答保存、跳题、获取下一题
- [x] Phase 4：人生小传生成
  - [x] 生成器实现（2000 字，按主题聚合）
  - [x] 预览触发条件判断（约 22-40 题）
  - [x] 初稿预览页面（两个入口）
...
### 📋 待开发
- Phase 5：补全机制（问题包补全、自由补全、更新人生小传）
- Phase 6：升级机制（主动触发、人生故事生成）
- Phase 8：用户中心与水滴管理 UI
- Phase 10：人生回忆录生成
...
# 运行特定测试
npm test -- verify-billing-formula
npm test -- ink-concurrency-lock
npm test -- lifecycle-utc-check
```
...
- **40 水滴的含义**：购买一轮基础访谈资格，不对应固定 40 题
- **提前结束**：如果 40 题左右内容已足够成文，可以提前结束，不为了凑满 50 题继续问
- **到达上限后的选择**：若 50 题位仍不够，再让老人自己选择生成当前版本、开启扩展包，或先停止
...
- **扩展操作预算**：保留 10 个可操作水滴
  - 第一次 AI 润色：免费
  - 第二次及以后每次润色：5 个水滴
  - 扩展题目包：10 个水滴开启 1 个最多 10 题的扩展包
  - 扩展包不是必须问满 10 题，中途内容够了也可以结束
...
- 题型原则：选择题为主，自由补充为辅
- 每道题必须独立承担一个内容槽位，不能只是下一道题的过渡
- 深挖题可以很多，但不应默认要求老人写长文本
...
*最后更新：2026-04-21*  
*版本：v0.3.0 - 强化"后端优先原则"，修正命令示例，明确文档同步规则*
